// CUDA WebShader 0.1.0. Generated from kernel initialise.
@group(0) @binding(0) var<storage, read_write> b_S: array<f32>;
@group(0) @binding(1) var<storage, read_write> b_W: array<f32>;
@group(0) @binding(2) var<storage, read_write> b_M: array<f32>;
@group(0) @binding(3) var<storage, read_write> b_V: array<f32>;
@group(0) @binding(4) var<storage, read_write> b_G: array<f32>;
@group(0) @binding(5) var<storage, read_write> b_Brain: array<f32>;
struct CWParams {
  p_seed: i32,
  cw_pad_4: u32,
  cw_pad_8: u32,
...
fn main(
  ...
  if ((v_i < 610i)) {
    ...
    if ((v_i >= 544i)) {
      ...
      cw_tmp_0 = ((f_hashf(f32((v_i + cw_params.p_seed)), cw_thread, cw_block, cw_grid) - 0.5f) * 0.1f);
    }
    b_W[v_i] = cw_tmp_0;
    b_M[v_i] = 0.0f;
    b_V[v_i] = 0.0f;
  ...
  if ((v_i < 640i)) {
    var v_field: i32 = (v_i % 16i);
    var v_variant: i32 = (v_i / 16i);
    var v_v: f32 = 0.0f;
    if ((v_field < 6i)) {
      v_v = (0.2f + (0.6f * f_hashf(f32((v_i + (cw_params.p_seed * 3i))), cw_thread, cw_block, cw_grid)));
    }
    if ((v_field == 8i)) {
      v_v = 1.0f;
    }
