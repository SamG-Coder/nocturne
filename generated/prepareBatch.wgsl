...
fn f_cw_buffer_helper_0(cw_buffer_arg_0: i32, cw_buffer_arg_1: i32, cw_thread: vec3<u32>, cw_block: vec3<u32>, cw_grid: vec3<u32>) -> bool {
  var cw_buffer_offset_0: i32 = cw_buffer_arg_0;
  var cw_buffer_offset_1: i32 = cw_buffer_arg_1;
  return (((((b_S[(cw_buffer_offset_0 + 8i)] == 1.0f) && (b_S[(cw_buffer_offset_0 + 51i)] > 0.5f)) && (b_S[(cw_buffer_offset_0 + 45i)] > 0.5f)) && ((i32(b_S[(cw_buffer_offset_0 + 7i)]) % 12i) == 0i)) && (b_Brain[(cw_buffer_offset_1 + 10i)] >= 40.0f));
}
...
fn main(
  ...
  if (((v_i >= 32i) || (!f_cw_buffer_helper_0(0i, 0i, cw_thread, cw_block, cw_grid)))) {
    return;
  }
  var v_count: i32 = i32(b_Brain[10i]);
  var v_available: i32 = (v_count - 5i);
  if ((v_available > (4096i - 5i))) {
    v_available = (4096i - 5i);
  }
  var cw_tmp_1: i32;
  if (((v_i % 2i) == 0i)) {
    var cw_tmp_0: i32;
    if ((v_available < 64i)) {
      cw_tmp_0 = v_available;
    } else {
      cw_tmp_0 = 64i;
    }
    cw_tmp_1 = cw_tmp_0;
  } else {
    cw_tmp_1 = v_available;
  }
  var v_span: i32 = cw_tmp_1;
  var v_age: i32 = i32((f_hashf(((b_S[7i] + f32((v_i * 173i))) + (b_Brain[0i] * 0.41f)), cw_thread, cw_block, cw_grid) * f32(v_span)));
  var v_seq: i32 = ((v_count - 6i) - v_age);
  var v_rb: i32 = (((v_seq + (4096i * 4i)) % 4096i) * 28i);
  var v_wb: i32 = (v_i * 88i);
  {
    var v_j: i32 = 0i;
    loop {
      if (!(v_j < 16i)) { break; }
      b_Work[(v_wb + v_j)] = b_R[(v_rb + v_j)];
      continuing {
        v_j += i32(1);
      }
    }
  }
  b_Work[(v_wb + 16i)] = b_R[(v_rb + 18i)];
  b_Work[(v_wb + 17i)] = b_R[(v_rb + 19i)];
}
