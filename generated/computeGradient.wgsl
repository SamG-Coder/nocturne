// CUDA WebShader 0.1.0. Generated from kernel computeGradient.
@group(0) @binding(0) var<storage, read> b_S: array<f32>;
@group(0) @binding(1) var<storage, read_write> b_Brain: array<f32>;
@group(0) @binding(2) var<storage, read> b_Work: array<f32>;
...
fn main(
  ...
  if (((v_i >= 610i) || (!f_cw_buffer_helper_0(0i, 0i, cw_thread, cw_block, cw_grid)))) {
    return;
  }
  var v_grad: f32 = 0.0f;
  {
    ...
    loop {
      ...
      var v_b: i32 = (v_n * 88i);
      if ((v_i < 512i)) {
        v_grad = (v_grad + (b_Work[((v_b + 52i) + (v_i / 16i))] * b_Work[(v_b + (v_i % 16i))]));
      } else {
        if ((v_i < 544i)) {
          v_grad = (v_grad + b_Work[(((v_b + 52i) + v_i) - 512i)]);
        } else {
          if ((v_i < 608i)) {
            v_grad = (v_grad + (b_Work[((v_b + 84i) + ((v_i - 544i) / 32i))] * b_Work[((v_b + 20i) + ((v_i - 544i) % 32i))]));
          } else {
            v_grad = (v_grad + b_Work[(((v_b + 84i) + v_i) - 608i)]);
          }
        }
      }
      continuing {
  ...
  b_Grad[v_i] = max((-0.8f), min(0.8f, cw_divide_f32(v_grad, f32(32i))));
  if ((v_i == 0i)) {
    var v_loss: f32 = 0.0f;
    {
      var v_n: i32 = 0i;
      loop {
        if (!(v_n < 32i)) { break; }
        v_loss = (v_loss + b_Work[((v_n * 88i) + 86i)]);
        continuing {
          v_n += i32(1);
        }
      }
    }
    b_Brain[5i] = cw_divide_f32(v_loss, f32(32i));
