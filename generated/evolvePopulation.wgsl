// CUDA WebShader 0.1.0. Generated from kernel evolvePopulation.
@group(0) @binding(0) var<storage, read> b_S: array<f32>;
@group(0) @binding(1) var<storage, read_write> b_G: array<f32>;
...
fn main(
  ...
  if ((((((cw_thread.x != u32(0i)) || (cw_block.x != u32(0i))) || (b_S[8i] != 1.0f)) || (b_S[45i] < 0.5f)) || (b_S[51i] < 0.5f))) {
    return;
  }
  if (((i32(b_S[7i]) % 1200i) != 0i)) {
    return;
  }
  {
    var v_species: i32 = 0i;
    loop {
      if (!(v_species < 5i)) { break; }
      var v_best: i32 = (v_species * 8i);
      var v_worst: i32 = (v_best + 1i);
      var v_hi: f32 = (-1.0f);
      var v_lo: f32 = 1000.0f;
      {
        var v_j: i32 = 0i;
        loop {
          if (!(v_j < 8i)) { break; }
          var v_g: i32 = ((v_species * 8i) + v_j);
          var v_score: f32 = b_G[((v_g * 16i) + 6i)];
          if (((b_G[((v_g * 16i) + 7i)] >= 2.0f) && (v_score > v_hi))) {
            v_hi = v_score;
            v_best = v_g;
          }
          if ((v_score < v_lo)) {
      ...
      if (((v_hi >= 0.0f) && (v_best != v_worst))) {
        {
          var v_p: i32 = 0i;
          loop {
            if (!(v_p < 6i)) { break; }
            b_G[((v_worst * 16i) + v_p)] = max(0.08f, min(0.92f, (b_G[((v_best * 16i) + v_p)] + ((f_hashf((b_S[7i] + f32(((v_p * 37i) + (v_species * 41i)))), cw_thread, cw_block, cw_grid) - 0.5f) * 0.25f))));
            continuing {
              v_p += i32(1);
            }
          }
        }
        {
          ...
          loop {
            if (!(v_p < 16i)) { break; }
            b_G[((v_worst * 16i) + v_p)] = 0.0f;
            continuing {
              v_p += i32(1);
            }
          }
        }
        b_G[((v_worst * 16i) + 9i)] = f32(v_best);
        b_G[((v_worst * 16i) + 6i)] = (v_hi * 0.45f);
        b_G[((v_worst * 16i) + 7i)] = 0.0f;
        b_G[((v_worst * 16i) + 8i)] = (b_G[((v_best * 16i) + 8i)] + 1.0f);
        b_Brain[12i] = (b_Brain[12i] + 1.0f);
        let cw_argument_index_0 = 13i;
        let cw_argument_index_1 = ((v_worst * 16i) + 8i);
        b_Brain[13i] = max(b_Brain[cw_argument_index_0], b_G[cw_argument_index_1]);
      }
      continuing {
        v_species += i32(1);
      }
