// CUDA WebShader 0.1.0. Generated from kernel recordExperience.
...
@group(0) @binding(3) var<storage, read_write> b_R: array<f32>;
...
fn main(
  ...
  if ((((b_S[8i] != 1.0f) || (b_S[51i] < 0.5f)) || ((i32(b_S[7i]) % 6i) != 0i))) {
    return;
  }
  var v_count: i32 = i32(b_Brain[10i]);
  var v_idx: i32 = (v_count % 4096i);
  var v_b: i32 = (v_idx * 28i);
  if ((v_count >= 5i)) {
    var v_older: i32 = (((v_count - 5i) % 4096i) * 28i);
    var v_tx: f32 = max((-1.5f), min(1.5f, cw_divide_f32((b_S[0i] - b_R[(v_older + 16i)]), 140.0f)));
    var v_ty: f32 = max((-1.5f), min(1.5f, cw_divide_f32((b_S[1i] - b_R[(v_older + 17i)]), 140.0f)));
    var v_error: f32 = (((v_tx - b_R[(v_older + 20i)]) * (v_tx - b_R[(v_older + 20i)])) + ((v_ty - b_R[(v_older + 21i)]) * (v_ty - b_R[(v_older + 21i)])));
    var v_baseline: f32 = (((v_tx - b_R[(v_older + 22i)]) * (v_tx - b_R[(v_older + 22i)])) + ((v_ty - b_R[(v_older + 23i)]) * (v_ty - b_R[(v_older + 23i)])));
    b_R[(v_older + 18i)] = v_tx;
    b_R[(v_older + 19i)] = v_ty;
    b_R[(v_older + 24i)] = 1.0f;
    b_R[(v_older + 26i)] = v_error;
    var cw_tmp_0: f32;
    if ((b_Brain[1i] < 1.0f)) {
      cw_tmp_0 = 1.0f;
    } else {
    ...
    var v_alpha: f32 = cw_tmp_0;
    let cw_argument_index_1 = 2i;
    b_Brain[2i] = f_mixf(b_Brain[cw_argument_index_1], v_error, v_alpha, cw_thread, cw_block, cw_grid);
    let cw_argument_index_2 = 3i;
    b_Brain[3i] = f_mixf(b_Brain[cw_argument_index_2], v_baseline, v_alpha, cw_thread, cw_block, cw_grid);
    b_Brain[1i] = (b_Brain[1i] + 1.0f);
    var cw_tmp_3: f32;
    if ((b_Brain[1i] > 64.0f)) {
      cw_tmp_3 = min(0.75f, max(0.0f, (1.0f - cw_divide_f32((b_Brain[2i] + 0.002f), (b_Brain[3i] + 0.002f)))));
    } else {
      cw_tmp_3 = 0.0f;
    }
    b_Brain[4i] = cw_tmp_3;
    b_Brain[7i] = min(f32((4096i - 5i)), f32((v_count - 4i)));
  }
  var v_p1: i32 = ((((v_count + 4096i) - 1i) % 4096i) * 28i);
  var v_p5: i32 = ((((v_count + 4096i) - 5i) % 4096i) * 28i);
  b_R[v_b] = cw_divide_f32(b_S[2i], 180.0f);
  b_R[(v_b + 1i)] = cw_divide_f32(b_S[3i], 180.0f);
  var cw_tmp_4: f32;
  if ((v_count > 0i)) {
    cw_tmp_4 = b_R[v_p1];
  } else {
  ...
  b_R[(v_b + 10i)] = cw_tmp_9;
  b_R[(v_b + 11i)] = cw_divide_f32(b_S[4i], b_S[5i]);
  var v_near: f32 = 1000000.0f;
  var v_dx: f32 = 0.0f;
  var v_dy: f32 = 0.0f;
  var v_density: f32 = 0.0f;
  {
    ...
    loop {
      if (!(v_j < 320i)) { break; }
      if ((b_E[((v_j * 48i) + 4i)] > 0.0f)) {
        var v_x: f32 = (b_E[(v_j * 48i)] - b_S[0i]);
        var v_y: f32 = (b_E[((v_j * 48i) + 1i)] - b_S[1i]);
        var v_d: f32 = ((v_x * v_x) + (v_y * v_y));
        if ((v_d < v_near)) {
          v_near = v_d;
          v_dx = v_x;
          v_dy = v_y;
  ...
  b_R[(v_b + 12i)] = max((-1.0f), min(1.0f, cw_divide_f32(v_dx, 300.0f)));
  b_R[(v_b + 13i)] = max((-1.0f), min(1.0f, cw_divide_f32(v_dy, 300.0f)));
  b_R[(v_b + 14i)] = min(1.0f, cw_divide_f32(v_density, 35.0f));
  b_R[(v_b + 15i)] = ((b_R[v_b] * b_R[(v_b + 3i)]) - (b_R[(v_b + 1i)] * b_R[(v_b + 2i)]));
  b_R[(v_b + 16i)] = b_S[0i];
  b_R[(v_b + 17i)] = b_S[1i];
  b_R[(v_b + 18i)] = 0.0f;
  b_R[(v_b + 19i)] = 0.0f;
  b_R[(v_b + 20i)] = b_Brain[8i];
  b_R[(v_b + 21i)] = b_Brain[9i];
  b_R[(v_b + 22i)] = max((-1.5f), min(1.5f, cw_divide_f32((b_S[2i] * 0.5f), 140.0f)));
  b_R[(v_b + 23i)] = max((-1.5f), min(1.5f, cw_divide_f32((b_S[3i] * 0.5f), 140.0f)));
  b_R[(v_b + 24i)] = 0.0f;
  b_R[(v_b + 25i)] = b_S[6i];
  b_Brain[10i] = (b_Brain[10i] + 1.0f);
  b_Brain[11i] = (b_Brain[11i] + 1.0f);
}
